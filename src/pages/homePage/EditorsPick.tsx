  ...
  useEffect(() => {
    ...
    const fetchEditorsPicks = async () => {
      try {
        ...
        // Cached Data
        const cachedData =
          localStorage.getItem("editorsPicks");

        if (cachedData) {
          setEditorsPicks(JSON.parse(cachedData));
          setLoading(false);
        }

        console.time("Editors Picks API");

        const res = await api.get("/editors-picks");

        console.timeEnd("Editors Picks API");
...
export default React.memo(EditorsPick);